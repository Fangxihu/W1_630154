...
ruleEntry appConnRules[] =
{
    ...
    RULE(RULE_EVENT_OUT_EAR,                    ruleOutOfEarLedsEnable,             CONN_RULES_LED_ENABLE),/*LED*/

    RULE(RULE_EVENT_IN_EAR,                     rulePeerSync,                       CONN_RULES_SEND_PEER_SYNC),
    RULE(RULE_EVENT_IN_EAR,                     ruleInEarLedsDisable,               CONN_RULES_LED_DISABLE),/*LED*/
...
static ruleAction ruleConnectHandsetStandardforLimitPeer(ruleConnectReason reason)
{
...
#ifdef LIMIT_PEER
				if (appPeerSyncPeerConnectedGet())
				{
					RULE_LOG("other peer connect fail, switch local out of case connect handset! by user!");
					return RULE_ACTION_RUN;
				}
#endif
...
static ruleAction ruleConnectHandsetforLimitPeer(ruleConnectReason reason)
{
    bdaddr handset_addr;
    ...
    if (appPeerSyncIsComplete())
    {
        ...
        else
        {
                ...
                (reason == RULE_CONNECT_OUT_OF_CASE))
            {
                ...
                if (ruleConnectHandsetStandardforLimitPeer(reason) == RULE_ACTION_RUN)
                {
...
#else
    return ruleConnectHandsetforLimitPeer(RULE_CONNECT_OUT_OF_CASE);
#endif
}
