...
static void ProcessOTAControlMessage(uint8 ota_msg_id, const uint8* payload, int payload_size)
{
    ...
    switch (ota_msg_id)
    {
        ...
        case SFWD_OTA_MSG_VOLUME_STOP:
            ...
            break;
#ifdef	LIMIT_PEER
        case SFWD_OTA_MSG_CONNECTED_FAIL:
            DEBUG_LOG("SFWD_OTA_MSG_CONNECTED_FAIL");
#if 0
			if(!appScoFwdPeerConnectedGet())
				appScoFwdHandlePeerConnectHandset();
            appScoFwdPeerConnectedMark();
#endif
            break;
#endif
        default:
...
#ifdef	LIMIT_PEER
#if 0
void appScoFwdPeerConnectedClear(void)
{
    scoFwdTaskData *theScoFwd = appGetScoFwd();

    theScoFwd->connected_fail = FALSE;
}

void appScoFwdPeerConnectedMark(void)
{
    scoFwdTaskData *theScoFwd = appGetScoFwd();

    theScoFwd->connected_fail = TRUE;
}

bool appScoFwdPeerConnectedGet(void)
{
    scoFwdTaskData *theScoFwd = appGetScoFwd();

    return theScoFwd->connected_fail;
}

void appScoFwdCallPeerConnectHandset(void)
{
    /*scoFwdTaskData *theScoFwd = appGetScoFwd();*/

    SendOTAControlMessage(SFWD_OTA_MSG_CONNECTED_FAIL);
    /*theScoFwd->peer_incoming_call = FALSE;*/
	appScoFwdOutOfCaseClear();
}

/******************************************/
void appScoFwdOutOfCaseClear(void)
{
    scoFwdTaskData *theScoFwd = appGetScoFwd();

    theScoFwd->out_of_case_handle = FALSE;
}

void appScoFwdOutOfCaseMark(void)
{
    scoFwdTaskData *theScoFwd = appGetScoFwd();

    theScoFwd->out_of_case_handle = TRUE;
}

bool appScoFwdOutOfCaseGet(void)
{
    scoFwdTaskData *theScoFwd = appGetScoFwd();

    return theScoFwd->out_of_case_handle;
}
/******************************************/

void appScoFwdHandlePeerConnectHandset(void)
{
    /*appConnRulesSetEvent(appGetSmTask(), RULE_EVENT_USER_SINGLE_RECON);*/
	/*appConManagerAllowHandsetConnect(TRUE);*/
	
	DEBUG_LOG("appScoFwdHandlePeerConnectHandset");
