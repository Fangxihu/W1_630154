 ...
 *  sent to the peer. */
/*!@{ */
#ifdef USER_PEER_SYNC_STATE
#define PEER_SYNC_MSG_SIZE                          16         /*!< Message length */
#else
...
#ifdef USER_PEER_SYNC_STATE
#define PEER_SYNC_MSG_USER_STATE_OFFSET             15
#endif
/*!@} */
...
#ifdef USER_PEER_SYNC_STATE
#define PEER_SYNC_MSG_USER_STATE_PEER_CONNECTED               (1 << 0)

/**/
#define PEER_SYNC_GET_USER_STATE(x)          (x[PEER_SYNC_MSG_USER_STATE_OFFSET])
#define PEER_SYNC_SET_USER_STATE(x,val)      (x[PEER_SYNC_MSG_USER_STATE_OFFSET] = val)

/**/
#define PEER_SYNC_USER_STATE_IS_PEER_CONNECTED(x) \
    ((PEER_SYNC_GET_USER_STATE(x) & PEER_SYNC_MSG_USER_STATE_PEER_CONNECTED) ? TRUE : FALSE)

/**/
#define PEER_SYNC_USER_STATE_SET_PEER_CONNECTED(x) \
    ((x) ? PEER_SYNC_MSG_USER_STATE_PEER_CONNECTED : 0)
...
#ifdef LIMIT_PEER
#if 0	
	if(appScoFwdOutOfCaseGet())
	{
		appScoFwdCallPeerConnectHandset();
	}
#endif
#endif
...
{
    peerSyncTaskData* ps = appGetPeerSync();
    bool current_peer_connected_state = ps->peer_connected_fail;

    ps->peer_connected_fail = peer_connected;
	
	/*数据解析，不适合在这里处理大量程序，所以task发送出去或者rule处理。类似中断并不做大量的实际处理*/
    if (!current_peer_connected_state && peer_connected)
    {
        DEBUG_LOGF("appPeerSyncUpdatePeerConnected Prev %u New %u", current_peer_connected_state, peer_connected);
		if(PHY_STATE_IN_CASE != appPhyStateGetState())
		{
			appPeerSyncHandlePeerConnectHandset();
		}
    }
    else if (current_peer_connected_state && !peer_connected)
    {
        DEBUG_LOGF("appPeerSyncUpdatePeerConnected Prev %u New %u", current_peer_connected_state, peer_connected);
        DEBUG_LOG("appPeerSyncUpdatePeerConnected Noting to do");
    }
}
#endif
...
#ifdef USER_PEER_SYNC_STATE
		const uint8 user_state = (PEER_SYNC_USER_STATE_SET_PEER_CONNECTED(appPeerSyncPeerConnectedGet()));
#endif
...
#ifdef USER_PEER_SYNC_STATE
		PEER_SYNC_SET_USER_STATE(message, user_state);
#endif
...
#ifdef USER_PEER_SYNC_STATE
    appPeerSyncUpdatePeerConnected(PEER_SYNC_USER_STATE_IS_PEER_CONNECTED(ind->msg));
#endif
...
bool appPeerSyncUserGetInCaseState(void)
{
    peerSyncTaskData* ps = appGetPeerSync();
    return ps->peer_in_case;
}

bool appPeerSyncUserGetPairingState(void)
{
    peerSyncTaskData* ps = appGetPeerSync();
...
void appPeerSyncPeerConnectedClear(void)
{
	DEBUG_LOG("appPeerSyncPeerConnectedClear---");
...
void appPeerSyncPeerConnectedMark(void)
{
	DEBUG_LOG("appPeerSyncPeerConnectedMark---");

    peerSyncTaskData* ps = appGetPeerSync();
    ps->peer_connected_fail = TRUE;
	appPeerSyncSend(FALSE);
}

bool appPeerSyncPeerConnectedGet(void)
{
	DEBUG_LOG("appPeerSyncPeerConnectedGet---");

    peerSyncTaskData* ps = appGetPeerSync();
    return ps->peer_connected_fail;
}

void appPeerSyncHandlePeerConnectHandset(void)
{
    /*appConnRulesSetEvent(appGetSmTask(), RULE_EVENT_USER_SINGLE_RECON);*/
	/*appConManagerAllowHandsetConnect(TRUE);*/
	
	DEBUG_LOG("appPeerSyncHandlePeerConnectHandset__");
