...
typedef struct
{
    ...
    bool peer_test_sync_flag:1;        /*!< The peer earbud is performing DFU. */
#endif
#ifdef PEER_SWTICH
    bool peer_switch_playing:1;        /*!< The peer earbud is performing DFU. */
#endif
#ifdef	LIMIT_PEER
	bool peer_connected_fail:1; 	   /*!< The peer earbud is performing DFU. */
#endif
} peerSyncTaskData;

#ifdef	LIMIT_PEER
enum peerSyncInternalMessages
{
    /*! Message sent to registered clients to notify of current peer sync state. */
    PEER_SYNC_INTERNAL_M1 = 0x52,
};
#endif
...
extern bool appPeerSyncUserGetInCaseState(void);

extern bool appPeerSyncUserGetPairingState(void);
