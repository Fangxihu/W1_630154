    ...
    switch (appA2dpGetState(theInst))
    {
		...
		DEBUG_LOGF("req->num_retries = %d", req->num_retries);
            ...
            if (appConManagerIsConnected(&theInst->bd_addr))
            {
				...
				DEBUG_LOGF("theInst->a2dp.connect_retries = %d", theInst->a2dp.connect_retries);

#ifdef	AUTO_ENTER_PAIR
            if ((req->num_retries == 0) && (TRUE == appSmHfpConnectFlagGet()) && (PHY_STATE_IN_CASE != appPhyStateGetState()))
...
#ifdef	LIMIT_PEER
				if(appPeerSyncPeerConnectedGet())/*判断是否为第一个7s*/
				{/*第二个7s*/
					if(!appPeerSyncUserGetInCaseState())/*判断peer是否在盒子里*/
					{/*peer不在盒子*/
						//appPeerSyncPeerConnectedClear();
						appSmUserHandsetEnterPairing();
					}
					else
					{/*peer在盒子*/
						appPeerSyncPeerConnectedMark();
						//appPeerSyncSend(FALSE);
					}
				}
				else	/*第一个7s，不管peer在不在盒子都发送，不做其他处理*/
				{
					appPeerSyncPeerConnectedMark();
					//appPeerSyncSend(FALSE);
				}
#else
#ifdef	SIMPLE_LIMIT_PEER
				if(!appPeerSyncUserGetInCaseState())/*对耳出盒的情况下才进配对*/
				{
					appSmUserHandsetEnterPairing();
				}
#else
            ...
            else
            {
                ...
                else
                {
                    ...
                    appA2dpSetState(theInst, A2DP_STATE_DISCONNECTED);
#ifdef	AUTO_ENTER_PAIR
			if((theInst->a2dp.disconnect_reason == AV_A2DP_CONNECT_FAILED) && (PHY_STATE_IN_CASE != appPhyStateGetState()) && (!appSmLinklossInd()))
			{
				DEBUG_LOG("A2DP enter pairing!!!!!!!!!!!!!!!!!!!!!!!!!");
#ifdef	LIMIT_PEER
				if(appPeerSyncPeerConnectedGet())/*判断是否为第一个7s*/
				{/*第二个7s*/
					if(!appPeerSyncUserGetInCaseState())/*判断peer是否在盒子里*/
