...
/*peer :x*(1000/625) ,x unit is ms
Range: 0x0001 to 0xFFFF
Time Range: 0.625 msec to 40.9 sec
final times = appConfigLeftEarbudPageTimeout * appConfigEarbudLinkLossPageTimeoutMultiplier

page_timeout * 625UL / 1000UL
*/
...
/*0x4000 = 30.72s*/
#define appConfigRightEarbudPageTimeout()   (0x2000)/*(0x2000)*/

/*! Page timeout to use for connecting to any non-peer Earbud devices. */
#ifdef	AUTO_ENTER_PAIR/*times. at creat acl will *4_link loss need *//*0xeee=2.4s one times*/
#define appConfigDefaultPageTimeout()       (0x0EEE)/*0x04B0*4=3s,0x0c80*4=2*4s,0xaaa=5s,0x1000=7.6s*/
#else
...
#define appConfigEarbudLinkLossPageTimeoutMultiplier()  (6)

#ifdef	AUTO_ENTER_PAIR
#define appConfigHandsetLinkLossPageTimeoutMultiplier() (12)
#else
...
#ifndef EQ_TUNING
#ifdef SW_DEBUG
#define appConfigChargerTerminationCurrent()           (1)
#else
#define appConfigChargerTerminationCurrent()           (20)
#endif
#else
#define appConfigChargerTerminationCurrent()           (1)
#endif
...
#undef appConfigInEarLedsEnabled()/*led ��in ear��ʱ�򲻹ر�=true*/
...
#define appConfigAvrcpConnectDelayAfterRemoteA2dpConnectMs() D_SEC(3)	/*D_SEC(3)*/

/*这两个时间不建议做更改，容易出现反复连断问题*/
/*! Time to wait to connect A2DP media channel after a locally initiated A2DP connection */
/*A2DP连接后，延时3s进行Media连接，音量同步也在这里*/
#define appConfigA2dpMediaConnectDelayAfterLocalA2dpConnectMs() D_SEC(3)	/*D_SEC(3)*/

/*! This timer is active in APP_STATE_OUT_OF_CASE_IDLE if set to a non-zero value.
    On timeout, the SM will allow sleep. */
#ifdef SW_DEBUG
#define appConfigIdleTimeoutMs()   D_SEC(60)
#else
